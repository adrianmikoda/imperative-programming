...
float calculate_value(Article *tab_article, const size_t n, const Date curr_date, const int days) {
    ...
    for (int i = 0; i < n; ++i){
        ...
        if (article_time > future_time) break;
        if (article_time == future_time) value += article.amount*article.price;
    }
    return value;	
}
